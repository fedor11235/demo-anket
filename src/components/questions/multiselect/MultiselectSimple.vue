<template>
  <div
    class="question question--select"
    ...
    <div class="question-list-wrapper">
      ...
      >
        <label
          ...
          @click.prevent="isActiveMeth3(value.id, value.title)"
        >
...
export default {
  ...
  methods: {
    isActiveMeth3(id, title) {
      const value = this.values.selects.find((elem) => elem.id === id);
      if (value) {
        value.isActive = !value.isActive;
        if (value.isActive) {
          for (const select of this.values.selects) {
            if (select.title === "Ничего из вышеперечисленного") {
              continue;
            }
            select.isActive = false;
          }
        } else {
          const value = this.values.selects.find(
            (elem) => elem.title === "Ничего из вышеперечисленного"
          );
          if (value) {
        ...
        this.values.selects.push({ id, isActive: true, title });
        if (title === "Ничего из вышеперечисленного") {
          for (const select of this.values.selects) {
            if (select.title === "Ничего из вышеперечисленного") {
              continue;
            }
